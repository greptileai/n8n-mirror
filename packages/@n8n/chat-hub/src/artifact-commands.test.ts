...
import { reconstructArtifacts, parseMessage } from './artifact-commands';
import type { MessageWithContent } from './artifact-commands';

describe(reconstructArtifacts, () => {
	describe('artifact creation', () => {
		test('should create a single artifact', () => {
...
`,
			...
			const result = reconstructArtifacts(messages);
		...
		test('should create multiple artifacts', () => {
			const messages: MessageWithContent[] = [
				{
					id: 'msg1',
					type: 'ai',
...
`,
		...
		test.todo('should return all artifacts when multiple commands in last message');

		// TODO: Fix implementation to return all artifacts
		// Currently only returns the last artifact when there are multiple complete commands in the last message
		// test('should return all artifacts when multiple commands in last message', () => {
		// 	const messages: MessageWithContent[] = [
		// 		{
		// 			id: 'msg1',
		// 			content: `
...
`;
	...
	describe('artifact editing', () => {
		test('should edit a artifact by title', () => {
			const messages: MessageWithContent[] = [
				{
					id: 'msg1',
					type: 'ai',
...
`,
		...
		test('should apply multiple edits to the same artifact', () => {
			const messages: MessageWithContent[] = [
				{
					id: 'msg1',
					type: 'ai',
...
`,
		...
		test('should edit specific artifact by title when multiple artifacts exist', () => {
			const messages: MessageWithContent[] = [
				{
					id: 'msg1',
					type: 'ai',
...
`,
		...
		test('should ignore edit command for non-existent artifact', () => {
			const messages: MessageWithContent[] = [
				{
					id: 'msg1',
					type: 'ai',
...
`,
		...
		test.todo(
			'should return both complete and incomplete artifacts when both exist in last message',
		);

		// TODO: Fix implementation to return both artifacts
		// Currently only returns the complete artifact when there's both complete and incomplete in the last message
		// test('should return both complete and incomplete artifacts when both exist in last message', () => {
		// 	const messages: MessageWithContent[] = [
		// 		{
		// 			id: 'msg1',
		// 			content: `
...
`;
			...
			const result = parseMessage({ id: 'test-msg', type: 'ai', content });
...
`;
			...
			assert(result[1].type === 'artifact-create');
...
`;
			...
			expect(result[1].type).toBe('artifact-create');
			expect(result[2].type).toBe('text');
			expect(result[3].type).toBe('artifact-create');
			expect(result[4].type).toBe('text');
		});

		test('should mark incomplete artifact-create commands correctly', () => {
			const content = `
Some text before.
...
<content>This is incomplete`;
		...
		test('should mark incomplete artifact-edit commands correctly', () => {
			const content = `
Some text before.
...
<newString>new text`;
			...
			assert(result[1].type === 'artifact-edit');
			expect(result[1].command.isIncomplete).toBe(true);
			expect(result[1].command.title).toBe('My Document');
...
<title>Doc Title</title>`;
			...
			expect(result).toHaveLength(1);
			expect(result[0].type).toBe('artifact-edit');
			assert(result[0].type === 'artifact-edit');
...
<oldString>old`;
		...
		describe('partial tag stripping during streaming', () => {
			...
			test('should strip trailing "<command:artifact-c" prefix', () => {
				const content = 'Some text here <command:artifact-c';
			...
			test('should strip trailing "<command:artifact-ed" prefix', () => {
				const content = 'Some text here <command:artifact-ed';
			...
			test('should parse complete opening tags as incomplete commands', () => {
				const content = 'Some text <command:artifact-create>';
			...
			test('should parse partial tag as text when too short to be command', () => {
				const content = '<command:artifact-c';
...
After command <command:artifact-ed`;
