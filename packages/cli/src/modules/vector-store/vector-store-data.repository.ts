...
import { BinaryDataService, FileLocation, type BinaryData } from 'n8n-core';
...
export class VectorStoreDataRepository {
	...
	private async getConnection(projectId: string): Promise<Connection> {
		...
		if (!connection) {
			...
			const storagePath = await this.binaryDataService.ensureLocation(this.getLocation(projectId));
			...
			if (storageConfig.mode === 's3') {
				...
				connection = await connect(storagePath);
			}

			this.logger.debug(`Created LanceDB connection for project: ${projectId} at ${storagePath}`);
	...
	/**
	 * Get total size of all vector stores (instance-wide) in bytes
	 */
	async getTotalSize(): Promise<number> {
		try {
			return await this.binaryDataService.getSize(this.getLocation());
		} catch (error) {
	...
	private getLocation(projectId?: string): BinaryData.FileLocation {
		return FileLocation.ofCustom({
			pathSegments: ['vector-store', this.provider].concat(
