...
import type { AddedNode, INodeUi } from '@/Interface';
...
import { hasProxyAuth, displayParameter as displayParameterUtil } from '@/app/utils/nodeIssueUtils';
...
export function useNodeHelpers() {
	...
	// Wrapper that resolves nodeTypeDescription from the store (maintains backwards compatibility)
	function displayParameter(
		...
		const nodeTypeDescription = node?.type
			...
			: null;
		return displayParameterUtil(nodeValues, parameter, path, node, nodeTypeDescription, displayKey);
	}
	...
	return {
		// Re-exported from nodeIssueUtils for backwards compatibility
		hasProxyAuth,
		displayParameter,
