...
describe('setupPanel.utils', () => {
	...
	describe('buildNodeSetupState', () => {
		...
		it('should build complete state for a fully configured non-trigger node', () => {
			const node = createNode({
				credentials: {
					testApi: { id: 'cred-1', name: 'Test' },
				},
		...
		it('should mark trigger as incomplete when credentials configured but not executed', () => {
			const node = createNode({
				credentials: {
					testApi: { id: 'cred-1', name: 'Test' },
				},
		...
		it('should mark trigger as complete when credentials configured and executed', () => {
			const node = createNode({
				credentials: {
					testApi: { id: 'cred-1', name: 'Test' },
				},
		...
		it('should mark trigger without credentials as complete after execution', () => {
			...
			const result = buildNodeSetupState(node, [], displayNameLookup, nodeNames, true, true);

			expect(result.isTrigger).toBe(true);
			expect(result.isComplete).toBe(true);
		});

		it('should mark trigger without credentials as incomplete before execution', () => {
			const node = createNode();
			const nodeNames = new Map<string, string[]>();

			const result = buildNodeSetupState(node, [], displayNameLookup, nodeNames, true, false);
